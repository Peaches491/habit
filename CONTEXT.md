...
Last updated at commit `aa224f8` (58 tests passing, CI green on `main`).
...
- `src/habit/web/app.py`: `create_app(config_path)` + `build_fields(config)`.
...
- `POST /checkin` is a **STUB** — echoes the submission under a "not yet scored
  or stored" banner. This is the seam for the next step.
- Run: `uv run python -m habit.web --config habit.yaml` (or `habit-web` script).

### Tests
`tests/test_config.py` (31), `tests/test_scoring.py` (22), `tests/test_web.py` (5).
...
- **`StorageAdapter` + Google Sheets adapter** (read/write *raw* answers;
  score/weekly writes are a disposable display cache). Needs Google
...
**Wire the `POST /checkin` handler.** Plan:
1. Parse the form → `DayLog`: checkbox present → `True` (absent → `False`),
   number string → int/float, select → choice string, textarea → free text;
   read the `_date` field for backfill.
2. Score with `score_day` and show line items / total / `pending` judged goals
   back on the confirmation page.
3. Persist — which pulls in the **`StorageAdapter` + Sheets** work, so POST
   wiring and the first storage adapter are effectively the same step.

**Open decision awaiting the user:** on submit, **score-and-display immediately**
(recommended — engine is ready, makes the form satisfying, surfaces pending
judged items) vs **capture raw only** and leave scoring to the recompute/agent
pass. Resolve this before implementing POST.
